...
static SceHidKeyboardReport k_reports[SCE_HID_MAX_REPORT];
...
VITA_PollKeyboard(void)
{
    ...
    if (keyboard_hid_handle > 0) {
        ...
        int numReports =
            sceHidKeyboardRead(keyboard_hid_handle, (SceHidKeyboardReport **) &k_reports, SCE_HID_MAX_REPORT);
